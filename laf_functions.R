...
calc_df_hivpop <- function(input, af, t, laf = 1, hm = 1){
  ...
  df <- 0
  df <- laf * fert_mult_by_age[af] * fert_mult_offart[hm] * ((hivstrat_adult[hm, af, 2, t] + hivstrat_adult[hm, af, 2, t-1]) / 2)  * mean(c(scalar[t], scalar[t-1]))
  df <- df + laf * fert_mult_by_age[af] * fert_mult_offart[hm] * ((artstrat_adult[1, hm, af, 2, t] + artstrat_adult[1, hm, af, 2, t-1]) / 2) * mean(c(scalar[t], scalar[t-1]))
...
calc_df_artpop <- function(input, af, t, laf, hm, hu){
  ...
  df = laf * fert_mult_onart[af] * ((artstrat_adult[hu, hm, af, 2, t] + artstrat_adult[hu, hm, af, 2, t-1]) / 2) * mean(c(scalar[t], scalar[t-1]))
  return(df)
}
...
calc_df_byaf <- function(input, af, t, laf = 1){
  ...
  nHIVcurr <- curr_last$nHIVcurr * scalar[t]
  nHIVlast <- curr_last$nHIVlast * scalar[t-1]
...
calc_hiv_births_age_spec <- function(input, laf, af, t){
  ...
  births <- (nHIVcurr * scalar[t] + nHIVlast * scalar[t-1]) / 2 * tfr[t] * df / (df * prev + 1 - prev) *  asfr[af, t] / (asfr_sum[t] * 5) ;
  return(births)
}
...
calc_hiv_births <- function(draw, laf, year.idx = 1:61, spec){
  input <- qs::qread(paste0(root, '/spectrum_data/draws/birth_prev/', draw, '.qs'))
  total_births = input$total_births[year.idx]
  input$asfr <- demp$asfr
  input$fert_mult_by_age <- hivp$fert_mult_by_age
  input$fert_mult_offart <- hivp$fert_mult_offart
  input$fert_mult_onart <- hivp$fert_mult_onart
  hiv = apply(input$hivstrat_adult[,,2,], 3, sum)
  input$scalar <- apply(spec$hivnum.f[4:10,], 2, sum) / hiv
